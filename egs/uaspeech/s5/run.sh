...
if [ ${stage} -le 3 ]; then
  utils/mkgraph.sh data/lang_largevocab_test exp/tri1 exp/tri1/graph_largevocab || exit 1

  steps/decode.sh --nj ${decode_nj} --cmd "$decode_cmd" \
    --num-threads 4 --parallel-opts "${parallel_opts}" \
    exp/tri1/graph_largevocab data/test exp/tri1/decode_largevocab_test || exit 1
fi
